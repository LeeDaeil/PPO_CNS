...
class ENVCNS(CNS):
    def __init__(self, Name, IP, PORT):
        super(ENVCNS, self).__init__(threrad_name=Name,
                                     ...
                                     Remote_IP='192.168.0.200', Remote_Port=PORT, Max_len=10)
    ...
    def send_act(self, A):
        ...
        if self.CMem.PZRLevl >= 99:                                         # 가압기 기포 생성 이전
            ...
            if self.PID_Mode:
                ...
                # A[0] HV142
                if A[0] == 0: self._send_control_save(ActOrderBook['LetdownValveStay'])
                if A[0] == 1: self._send_control_save(ActOrderBook['LetdownValveClose'])
                if A[0] == 2: self._send_control_save(ActOrderBook['LetdownValveOpen'])
            ...
            if self.PID_Mode:
                ...
                # Spray ----------------------------------------------------------
                # A[0] PZR spray ## 12.14
                if A[0] == 0: self._send_control_save(ActOrderBook['PZRSprayStay'])
                if A[0] == 1: self._send_control_save(ActOrderBook['PZRSprayClose'])
                if A[0] == 2: self._send_control_save(ActOrderBook['PZRSprayOpen'])
